...
function OrganizationProfile() {
  ...
  useEffect(() => {
    if (selectedOrg) {
      setIsPrimaryOwner(selectedOrg.is_primary === true);
      getOrganizationProfile(selectedOrg.id);
    }
  }, [selectedOrg]);
  ...
  const getOrganizationProfile = async (organizationId) => {
    try {
      if (!organizationId) {
        setMessage({
          type: 'error',
          text: 'No organization selected. Please select an organization from the dropdown.'
        });
        return;
      }
  ...
  const updateOrganizationProfile = async () => {
    try {
      ...
      if (!isPrimaryOwner) {
        setMessage({
          type: 'error',
          text: 'You do not have permission to update this organization. Please contact the primary owner.'
        });
        return;
      }
      ...
      // Call the API endpoint
      const response = await fetch(`${apiUrl()}/organizations/${selectedOrg.id}`, {
        method: 'PUT',
        headers: {
          'accept': 'application/json',
  ...
  // Show non-primary owner message
  const renderNonPrimaryOwnerMessage = () => {
    if (!selectedOrg) return null;
    
    if (!isPrimaryOwner) {
      return (
        <div className="p-6 rounded-xl flex flex-col items-center text-center bg-amber-900/30 text-amber-200 border border-amber-700">
          <div className="mb-4">
            <FiAlertCircle className="text-amber-400 text-4xl" />
          </div>
          <h3 className="text-xl font-bold mb-2">You are not the primary owner of this organization</h3>
          <p className="mb-4 max-w-2xl">
            Only the primary owner can view and edit organization details. Please contact the primary owner if you need to update this information.
          </p>
          <div className="text-sm text-amber-400/80 bg-amber-950/50 p-3 rounded-lg max-w-2xl">
            <p>Organization: <span className="font-medium">{selectedOrg.org_name}</span></p>
            {selectedOrg.org_email && <p className="mt-1">Contact: <span className="font-medium">{selectedOrg.org_email}</span></p>}
          </div>
        </div>
      );
    }
  ...
  return (
    <>
      ...
      {!selectedOrg && (
        <div className="mb-6 p-4 rounded-xl flex items-start bg-blue-900/30 text-blue-200 border border-blue-700">
          <div className="mr-3 mt-0.5">
            <FiAlertCircle className="text-blue-400" />
          </div>
          <div>
            <p>Please select an organization from the dropdown to view and manage its details.</p>
          </div>
        </div>
      )}

      {/* Non-primary owner message */}
      {selectedOrg && !isPrimaryOwner ? (
        renderNonPrimaryOwnerMessage()
      ) : (
        selectedOrg && isPrimaryOwner && (
